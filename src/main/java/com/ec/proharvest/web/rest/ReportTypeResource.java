...
public class ReportTypeResource {
    ...
    public ReportTypeResource(ReportTypeService reportTypeService) {
    ...
    public ResponseEntity<List<ReportTypeDTO>> getAllReportTypes(Pageable pageable) {
        log.debug("REST request to get a page of ReportTypes");
        Page<ReportTypeDTO> page = reportTypeService.findAll(pageable);
    ...
    public ResponseEntity<List<ReportTypeDTO>> searchReportTypes(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ReportTypes for query {}", query);
        Page<ReportTypeDTO> page = reportTypeService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
